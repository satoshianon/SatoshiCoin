Satoshi Coin version 0.18.1 is now available from:

  <https://download.satoshicoin.pro/satoshicoin-0.18.1/>
...
`/Applications/SatoshiCoin-Qt` (on Mac) or `satoshicoind`/`satoshicoin-qt` (on
Linux).
...
Satoshi Coin is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Satoshi Coin on unsupported systems.

Satoshi Coin should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, Satoshi Coin does not yet change appearance when
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/satoshianon/SatoshiCoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@satoshicoin.pro) the Satoshi Coin
  project to let us know about their use-case, as this feature could be
  deprecated in the future).

Wallet changes
--------------
When creating a transaction with a fee above `-maxtxfee` (default 0.1 SAT),
the RPC commands `walletcreatefundedpsbt` and  `fundrawtransaction` will now fail
instead of rounding down the fee. Beware that the `feeRate` argument is specified
in SAT per kilobyte, not litoshi per byte.

Documentation
-------------

- A new short [document](https://github.com/satoshianon/SatoshiCoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/satoshianon/SatoshiCoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/satoshianon/SatoshiCoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/satoshianon/SatoshiCoin/blob/master/doc/satoshicoin-conf.md)
  about the `satoshicoin.conf` file describes how to use it to configure
  Satoshi Coin.

- A new document introduces Satoshi Coin's BIP174 [Partially-Signed
  Satoshi Coin Transactions
  (PSBT)](https://github.com/satoshianon/SatoshiCoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/satoshianon/SatoshiCoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent SatoshiCoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Satoshi Coin to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/satoshianon/SatoshiCoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/satoshianon/SatoshiCoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Satoshi Coin knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Satoshi Coin, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/satoshianon/SatoshiCoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/satoshianon/SatoshiCoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart satoshicoind with the `-deprecatedrpc=generate`
...
- A new `satoshicoin-wallet` tool is now distributed alongside Satoshi Coin
...
This section describes planned changes to Satoshi Coin that may affect
other Satoshi Coin software and services.

- Since version 0.16.0, Satoshi Coin’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Satoshi Coin 0.20 (expected about
  a year after 0.18), Satoshi Coin will default to native segwit
  ...
  to bech32 addresses, and if the Satoshi Coin project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Satoshi Coin 0.20.
  ...
  set the `addresstype=bech32` configuration option in any Satoshi Coin
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Satoshi Coin
...
- A sub-project of Satoshi Coin now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Satoshi Coin.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Satoshi Coin's own implementation, although entropy
  gathered by Satoshi Coin is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Satoshi Coin a
...
- On macOS, Satoshi Coin now opts out of application CPU throttling
