Satoshi Coin version 0.13.2 is now available from:

  <https://download.satoshicoin.pro/satoshicoin-0.13.2.1/>
...
updates will be released anymore. Without security updates, using a satoshicoin
...
ECDSA signatures inside Satoshi Coin transactions now use validation using
...
Previous versions of Satoshi Coin had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Satoshi Coin was to raise the default minimum
relay fee.

Satoshi Coin 0.13.2 will have a strict maximum size on the mempool. The
...
Satoshi Coin 0.13.2 also introduces new default policy limits on the length and
...
This allows running satoshicoind without having to do any manual configuration.
...
Satoshi Coin has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Satoshi Coin will relay transactions with
...
entirely in Satoshi Coin 0.13, and it is at this time undecided whether the
...
Satoshi Coin has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Satoshi Coin automatically creates a hidden service to listen on, without
manual configuration. Satoshi Coin will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Satoshi Coin is listening, and
...
Satoshi Coind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Satoshi Coin
Core to use floating fees. By default, Satoshi Coin will use floating
fees.
...
At all times, Satoshi Coin will cap fees at `-maxtxfee=<x>` (default:
0.10) SAT.
Furthermore, Satoshi Coin will never create transactions paying less than
...
`satoshicoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /satoshicoinrpc>
        ...
        # optional bypass satoshicoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< satoshicoinrpc:<password>
    </Location>
...
Restarting satoshicoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in satoshicoin.conf
...
satoshicoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/satoshicoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Satoshi Coin code base has
...
- `satoshicoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `satoshicoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Satoshi Coin scripting system that in combination with
...
Satoshi Coin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Satoshi Coin.
...
The two phases are distinct in the SatoshiCoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Satoshi Coin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Satoshi Coin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Satoshi Coin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of satoshicoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of satoshicoins being spent is exactly
...
- **Increased security for multisig:** Satoshi Coin addresses (both P2PKH addresses
  ...
  believed to be provided by Satoshi Coin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Satoshi Coin's current ledger (called the UTXO set),
  ...
  note that Satoshi Coin does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Satoshi Coin users to individually opt-in to almost any change in the Satoshi Coin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Satoshi Coin contributors that may
...
segwit's start date of 1 January 2017 miners can update the Satoshi Coin
client to Satoshi Coin 0.13.2 to signal for segwit support. When a
...
Since Satoshi Coin 0.10.0, nodes have defaulted to only relaying and
...
- `satoshicoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `satoshicoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
