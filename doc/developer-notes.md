...
- [Development guidelines](#development-guidelines)
    - [General Satoshi Coin](#general-satoshi-coin)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Satoshi Coin uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play satoshicoins" on the test network, if you
...
Satoshi Coin is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/satoshianon/SatoshiCoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_satoshicoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_satoshicoin --log_level=test_suite
$ valgrind -v --leak-check=full src/satoshicoind -printtoconsole
...
# A coverage report will now be accessible at `./test_satoshicoin.coverage/index.html`.
```
...
To profile a running satoshicoind process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep satoshicoind` -- sleep 60
```
...
Satoshi Coin can be compiled with various "sanitizers" enabled, which add
...
- Net threads:
  ...
  - [ThreadSocketHandler (`b-net`)](https://doxygen.bitcoincore.org/class_c_connman.html#a765597cbfe99c083d8fa3d61bb464e34)
    : Sends/Receives data from peers on port 14742.
...
However, in open source software such as Satoshi Coin, where everyone uses
...
pay attention to for reviewers of Satoshi Coin code.

General Satoshi Coin
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: Satoshi Coin uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Some of these are maintained by active developers of Satoshi Coin, in which case changes should probably go
directly upstream without being PRed directly against the project. They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project. Changes to these should also
be sent upstream, but bugfixes may also be prudent to PR against Satoshi Coin so that they can be integrated
...
file descriptors, it will cause problems with Satoshi Coin's `select()` loop, because
...
compatibility between nodes. This happened in Satoshi Coin 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to Satoshi Coin's
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `satoshicoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `satoshicoin-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Satoshi Coin, the wallet was always
